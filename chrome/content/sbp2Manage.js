...
//			iecdPathField.label = iecdPickerDir.file.path;
		...
		if ( ieidAnswer == true ) {
			...
			for ( var ieidI=0; ieidI<ieidNumRanges; ieidI++)
			{
				...
				for ( var ieidJ=ieidStart.value; ieidJ<=ieidEnd.value; ieidJ++)
				{
					...
					//exportierte Einträge aus sbp1 sind Verzeichnisse, ab sbp2 handelt es sich um ZIP-Dateien.
	...
	ieItemExport : function()
	{
		...
		for (var iieI=0; iieI<iieNumRanges; iieI++)
		{
			...
			for (var iieJ=iieStart.value; iieJ<=iieEnd.value; iieJ++)
			{
				...
				//Sonderbehandlung für Verzeichnisse
				if ( iieType=="folder" ) {
					//alle Einträge des Verzeichnis zum Exportieren vormerken. Das Verzeichnis selbst wird nicht exportiert.
					iieResList = this.ieItemExportContainerAdd(iieData, iieResList, iieRes);
				} else {
					//Sicherstellen, dass die Resource noch neu ist. Sie könnte schon bei der Sonderbehandlung für Verzeichnisse hinzugekommen sein.
					iieExists = 0;
	...
	{
		//Bei iiecaRes handelt es sich um ein Verzeichnis. Die Funktion nimmt alle Einträge des Verzeichnis in iiecaResList auf. Unterverzeichnisse werden durch rekursiven Aufruf dieser
		//Funktion ebenfalls berücksichtigt.
		//
		//Ablauf:
		//1. Container initialisieren
		//2. Enumerator erstellen
		//3. Alle Einträge aufnehmen (keine seperator und folder)
		//4. Liste mit Einträgen zurück an aufrufende Funktion
		...
		while ( iiecaContEnum.hasMoreElements() )
		{
			...
			//Bei Verzeichnissen wird diese Funktion rekursiv aufgerufen, "seperator" werden ignoriert, alle anderen Einträge aufgenommen
			if ( iiecaType == "folder" ) {
